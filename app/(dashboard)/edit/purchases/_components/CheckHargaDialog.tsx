...
import { useGetLatestPurchasedItemsByClient } from "@/modules/purchase/purchase.queries";
...
export default function CheckHargaDialog({ clientId }: { clientId: number }) {
  ...
  const { data: items, isFetching } = useGetLatestPurchasedItemsByClient(
    clientId,
    normalizedName,
    !!normalizedName,
  );
    ...
    <FormProvider {...methods}>
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          {!isFetching && !!normalizedName && items?.length === 0 && (
            <p className="text-sm text-muted-foreground">
            ...
            (items || []).map((item) => (
              <div key={item.name} className="rounded-md border p-3">
                ...
                <p className="text-xs text-muted-foreground">
                  {dayjs(item.bought_at).format("DD/MM/YYYY HH:mm")}
                </p>
              </div>
            ))}
        </div>
