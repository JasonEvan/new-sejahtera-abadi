...
import { useGetReceivablesByClient } from "@/modules/report/report.queries";
import { useEffect, useState } from "react";
import { FormProvider, useForm } from "react-hook-form";
import { toast } from "sonner";
import { columns } from "./columns";

export default function ReceivablesPerClientContent() {
  ...
  const {
    ...
    error: resultError,
  } = useGetReceivablesByClient(clientId, !!clientId);
  ...
  useEffect(() => {
    if (isResultError) {
      toast.error(resultError.message || "Gagal memuat data piutang client", {
        position: "bottom-right",
      });
    }
        ...
        <form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button
              type="button"
              onClick={() =>
                printService.handlePrintReceivablesPerClient(
                  receivables || [],
                  selectedClientName,
                )
              }
