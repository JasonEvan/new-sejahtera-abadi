...
import { useDeletePurchaseMutation } from "@/modules/purchase/purchase.mutations";
...
export default function InvoiceSummary() {
  const { invoice_information, meta } = useDeletePurchaseStore();
  const deletePurchaseMutation = useDeletePurchaseMutation();

  if (!invoice_information.purchase_order_id) return null;

  const handleDelete = async () => {
    deletePurchaseMutation.mutate(invoice_information.purchase_order_id);
  };
  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        ...
        <CardDescription>
          Informasi detail nota pembelian yang akan dihapus
        </CardDescription>
      </CardHeader>
        ...
        <Button
          ...
          onClick={() =>
            alertDialogs.open({
              ...
              description: `Apakah Anda yakin ingin menghapus nota pembelian ${invoice_information.invoice_number}? Tindakan ini akan menghapus semua data transaksi terkait secara permanen dan tidak dapat dibatalkan.`,
              confirmText: "Hapus",
