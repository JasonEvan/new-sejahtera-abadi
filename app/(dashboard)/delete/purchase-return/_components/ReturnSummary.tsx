...
import { useGetEditPurchaseReturnDetail } from "@/modules/purchase-return/purchase-return.queries";
import { useDeletePurchaseReturnMutation } from "@/modules/purchase-return/purchase-return.mutations";
...
export default function ReturnSummary() {
  const { transaction_information } = useDeletePurchaseReturnStore();
  const deleteReturnMutation = useDeletePurchaseReturnMutation();

  const { data: detail, isLoading } = useGetEditPurchaseReturnDetail(
    transaction_information.purchase_return_id,
    !!transaction_information.purchase_return_id,
  );

  if (!transaction_information.purchase_return_id) return null;
  ...
  const handleDelete = async () => {
    deleteReturnMutation.mutate(transaction_information.purchase_return_id);
  };
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-destructive/10 p-4 rounded-lg">
          <p className="text-sm text-destructive font-medium">
            ...
            dalam nota dan menambah saldo hutang klien. Tindakan ini tidak dapat
            dibatalkan.
          </p>
        </div>
      </CardContent>
        ...
        <Button
          ...
          onClick={() =>
            alertDialogs.open({
              ...
              description: `Apakah Anda yakin ingin menghapus retur untuk nota ${invoiceNumber} tanggal ${dayjs(returnDate).format("DD MMMM YYYY")}? Saldo hutang klien akan bertambah.`,
              confirmText: "Hapus",
