...
export const deleteEditReceivablesByInvoiceValidation = z.object({
  invoice_number: z.string().trim().min(1, "Nomor nota harus diisi"),
});

export const updateEditReceivablesByInvoiceValidation = z
  .object({
    transaction_number: z.string().trim().min(1, "Nomor transaksi harus diisi"),
    payments: z
      .array(
        z.object({
          invoice_number: z.string().trim().min(1, "Nomor nota harus diisi"),
